...
namespace Movie.Application.Services
{
    public class UpvoteService : IUpvoteService
    {
        ...
        public GetUpvoteRatingDto UpvoteRatingDtoByFilmId(Guid filmId)
        {
            ...
            decimal totalValue = upvotes.Sum(x => x.Value);
            int qtde = upvotes.Count();

            GetUpvoteRatingDto getUpvoteRatingDto = new GetUpvoteRatingDto
            {
                Value = 5
            };

            if (qtde == 0)
                return getUpvoteRatingDto;

            getUpvoteRatingDto.Value = (decimal)totalValue / qtde;
